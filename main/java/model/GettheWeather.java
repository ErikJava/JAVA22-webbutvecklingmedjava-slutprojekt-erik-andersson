...
public class GettheWeather {
	...
	public static void getWeather(WeatherBean wBean) throws IOException {
		String API_KEY = "d12c7abf94b22772647ea334379f004a";
		...
		for (int temp = 0; temp < nList.getLength(); temp++) {
			...
			// gets the date
			NodeList nListii = doc.getElementsByTagName("lastupdate");

			// loop through the content of the tag
			for (int tempii = 0; tempii < nListii.getLength(); tempii++) {
				// Save a node of the current list id
				Node nodeii = nListii.item(temp);
				if (nodeii.getNodeType() == Node.ELEMENT_NODE) {

					// set the current node as an Element
					Element eElementii = (Element) nodeii;
					// get the content of an attribute in element
					String XMLlastupdate = eElementii.getAttribute("value");
					// and print it
					System.out.println(wBean.getCityStr() + " is now a " + XMLlastupdate);
					// save it
					wBean.setDateStr(XMLlastupdate);
